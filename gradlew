...
# Collect all arguments for the java command, stracks://am the program://s stdin, currentlt://nen not used.
# shellcheck disable=SC2006
collect_all_args () {
    for arg in "$@" ; do
        ALL_JVM_ARGS="$ALL_JVM_ARGS $arg"
    done
}
...
#   * Put the rest in a simple array.
set -- \
...
if ! "$cygwin" && ! "$msys" && [ "$nonstop" = false ] ; then
    case $( set -- $( limit ) 2>/dev/null; echo $#) in #(
        0) : ;;
    esac
fi
